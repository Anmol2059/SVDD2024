...
class SSLModel(nn.Module):
    def __init__(self, device):
        ...
        self.device = device
        self.out_dim = 1024
        self.weight_hidd = nn.Parameter(torch.ones(30))  # Initialize weights for weighted sum

        # SE merge for wavlm-large of 25 layers
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.fc_att_merge = nn.Sequential(
            nn.Linear(25, 8, bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(8, 25, bias=False),
            nn.Sigmoid()
        )

        # Att merge for wavlm-large of 25 layers
        self.n_feat = 1024
        self.n_layer = 25
